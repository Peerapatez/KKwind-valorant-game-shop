...
$tyId = $_GET['type'];
$stmt = $db->prepare("SELECT * FROM producttype WHERE type_id = $tyId");
$stmt->execute();
...
<div class="lg:hidden hidden" role="dialog" aria-modal="true" id="mobileMenu">
   ...
   <section class="custom-bg p-6" style=" min-height: 100vh;">
    <div class="container borderbanner">
      ...
      <img src="<?php echo $productType['type_img'] ?>" alt="รูปภาพสินค้า" class="imgb">
      <h1 class="text-4xl text-white fontm"><?php echo $productType['type_name'] ?></h1>
      <p style="color: aliceblue; margin-left: 5% ;margin-right: 4.9%; margin-bottom: 40px;"><?php echo $productType['type_desc'] ?></p>
      
      <div class="grid grid-cols-12 sm:grid-cols-2 lg:grid-cols-3 ">
        <?php
        $typeId = $_GET['type'];
        $getProduct = $db->query("SELECT * FROM product WHERE type_id = '$typeId'");

        while($products = $getProduct->FETCH(PDO::FETCH_ASSOC)){
        ?>
        <div class="card-skin sm:w-1/2 lg:w-1/3 p-3">
          <div>
            <h5 class=" text-center py-2">
              <span><i class="fas fa-coins"></i> <?php echo $products['product_price'] ?> Point!</span>
            </h5>
            <img src="<?php echo $products['product_img'] ?>" class="card-img-top w-full">
          </div>
          <div class="my-3 text-start px-3">
            <div class="row">
              <div class="col-12 text-start">
                <h5 class="text-lg font-bold mt-3"><?php echo $products['product_name'] ?></h5>
                <h5 class="text-md mt-3"><?php echo $products['product_price'] ?> ฿ </h5>
              </div>
              <div class="col-12">
...
document.querySelectorAll('#buyButton').forEach(function(button) {
    ...
    const price = button.closest('.card-skin').querySelector('h5.text-md').innerText.replace(' ฿', '').trim();
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            xhr.send("user_id=" + encodeURIComponent(user_id) + "&price=" + encodeURIComponent(price));
        }
    });
});
});
